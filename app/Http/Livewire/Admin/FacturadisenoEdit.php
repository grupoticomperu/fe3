...
class FacturadisenoEdit extends Component
{
    ...
    public function mount(Facturadiseno $facturadiseno)
    {
        $this->company = auth()->user()->employee->company;

        $this->facturadiseno = $facturadiseno;
        // Cargar datos del transportista
        $this->name = $facturadiseno->name;
        $this->nameblade = $facturadiseno->nameblade;
        $this->image1back = $facturadiseno->image1;
        $this->image2back = $facturadiseno->image2;
        $this->order = $facturadiseno->order;
        $this->description = $facturadiseno->description;
        $this->state = $facturadiseno->state;
    }
    ...
    protected function rules()
    {
        return [
            ...
            'nameblade' => [
                ...
                'string',
                'max:255',
                Rule::unique('facturadisenos', 'nameblade')->ignore(optional($this->facturadiseno)->id),
            ],
    ...
    public function update()
    {
        ...
        $this->emit('alert', 'El Diseño de Factura se actualizó correctamente');
        return redirect()->route('admin.facturadiseno.list');
    }

    public function render()
    {
        return view('livewire.admin.facturadiseno-edit');
    }
}
