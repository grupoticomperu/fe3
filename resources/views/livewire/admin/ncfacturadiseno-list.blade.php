<div wire:init="loadNcfacturadisenos">
    ...
    <div class="container py-12 mx-auto border-gray-400 max-w-7xl sm:px-6 lg:px-8">
        <div class="items-center px-6 py-4 bg-gray-200 sm:flex">
            ...
            <div class="flex items-center justify-center">
                <a href=" {{ route('admin.ncfacturadiseno.create') }} "
            ...
            @if (count($ncfacturasdisenos))
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @foreach ($ncfacturasdisenos as $ncfacturasdiseno)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ $ncfacturasdiseno->name }}

                                </td>

                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                    {{ $ncfacturasdiseno->nameblade }}
                                </td>


                                <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">

                                    @if ($ncfacturasdiseno->image1)
                                        <img src="{{ Storage::disk('s3_public')->url($ncfacturasdiseno->image1) }}"
                                            width="200" alt="">
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @switch($ncfacturasdiseno->state)
                                        @case(0)
                                            {{-- @canboletasdisenos Update') --}}
                                            <span wire:click="activar({{ $ncfacturasdiseno->id }})"
                                        ...
                                        @case(1)
                                            {{-- @canboletasdisenos Update') --}}
                                            <span wire:click="desactivar({{ $ncfacturasdiseno->id }})"
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                    ...
                                    {{-- @can('Boletadiseno Update') --}}
                                         <a href=" {{ route('admin.ncfacturadiseno.edit', $ncfacturasdiseno->id) }} "
                                            class="btn btn-green">
                                   ...
                                   {{--  @endcan
                                    ...
                                    @can('Boletadiseno Delete') --}}
                                        <a class="btn btn-red"
                                            wire:click="$emit('deleteNcfacturadiseno', {{ $ncfacturasdiseno->id }}) ">
                ...
                @if ($ncfacturasdisenos->hasPages())
                    <div class="px-6 py-4">
                        {{ $ncfacturasdisenos->links() }}
                    </div>
                @endif
            @endif
        </x-table>
    ...
    @push('scripts')
        ...
        <script>
            Livewire.on('deleteNcfacturadiseno', ncfacturasdisenoId => {
                Swal.fire({
                    title: 'Estas seguro?',
                    ...
                    if (result.isConfirmed) {
                        ...
                        Livewire.emitTo('admin.ncfacturadiseno-list', 'delete', ncfacturasdisenoId);
